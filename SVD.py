...
def modeMaker(path):
    ...
    print(f"Number of available/used snapshots: {len(times)}/{len(window_times)}")
...
data_matrix = pt.cat((Re142,Re198,Re302),0)#,Re392), 0)     
svd = SVD(data_matrix)
...
modeCoeff = pt.cat(( modeCoeff2, modeCoeff3, modeCoeff4),0)#, modeCoeff5),0)
print("modeCoeff",modeCoeff.shape)
print("modeCoeff2",modeCoeff2.shape)
print("modeCoeff3",modeCoeff3.shape)
print("modeCoeff4",modeCoeff4.shape)
#print("modeCoeff5",modeCoeff5.shape)

pt.save(svd.s,f"{data_save}svds.pt")
pt.save(lenDataset,f"{data_save}lenDataset.pt")
