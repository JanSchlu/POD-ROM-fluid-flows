...
def predictor_residual_period(data):
    predict = pt.ones([len(data)-1,1])                                           # pred len(test_data)-1-p_steps
    predict[0] = data[1]
    for i in range (1, len(predict)):
        predict[i] = predict[i-1] + 0.01 # modelR(data[i])
    predict = predict.detach().numpy()
    return predict



    predStore = pt.ones([len(data),1])                 
    predicted = pt.ones([len(data),1])
